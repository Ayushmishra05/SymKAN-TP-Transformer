...
class Seq2Seq(nn.Module):
    def __init__(self, p, embedding, encoder, decoder, pad_idx):
        ...
        self.p = p
        # Add KAN layer: input is hidden dimension (d_x), output is vocabulary size (d_vocab)
    ...
    def get_interpretability(self, src, trg):
        """Get interpretability info from the KAN layer."""
        self.eval()
        with torch.no_grad():
            src_mask, trg_mask = self.make_masks(src, trg)
            src = self.embedding(src)
...
class KANLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_knots=5, spline_order=3):
        ...
        # Total knots needed: num_knots + spline_order to handle boundary conditions
        total_knots = num_knots + spline_order
        self.knots = torch.linspace(-1, 1, total_knots)

        # Coefficients for B-spline basis functions (inner layer)
        self.inner_coeffs = nn.Parameter(
            torch.randn(in_dim, num_knots) * 0.1
        )

        # Coefficients to combine inner functions for each output dimension (outer layer)
    ...
    def forward(self, x):
        """Map input to logits using KAN."""
        batch_size, seq_len, in_dim = x.shape
        assert in_dim == self.in_dim, f"Input dimension mismatch: expected {self.in_dim}, got {in_dim}"

        # Normalize input to [-1, 1] for B-spline stability
        x = torch.tanh(x)

        # Compute inner functions (B-spline basis for each input dimension)
        ...
        # Compute outer functions (sum of inner outputs for each output dimension)
    ...
    def get_interpretability(self, x):
        ...
        # Compute contributions per output dimension
