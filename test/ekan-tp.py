...
class KANLinear(nn.Module):
    def __init__(
        ...
        scale_noise=0.1,
        scale_base=1.0,
        scale_spline=0.1,  # Reduced to stabilize
...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_heads, dim, src_dim, dropout, normalized_attention, xav_init=False):
        super().__init__()
        self.layer_id = id(self)  # Unique identifier for caching
        self.dim = dim
    ...
    def forward(self, input, mask, kv=None, use_cache=False, first_loop=True):
        ...
        # Adjust mask reshape to ensure qlen and klen match the input
        if mask is not None:
            if mask.dim() == 4:
                mask_reshape = mask.size()  # [bs, 1, qlen, klen]
                if mask_reshape[2] != qlen or mask_reshape[3] != klen:
                    # Trim or pad mask to match qlen and klen
                    mask = mask[:, :, :qlen, :klen]
                    mask_reshape = mask.size()
            elif mask.dim() == 3:
        ...
        if mask is not None:
            mask = (mask == 0).view(mask_reshape).expand(-1, n_heads, -1, -1)
...
class TransformerLayer(nn.Module):
    def __init__(self, params, is_encoder, gated=False, is_last=False):
        ...
        if self.is_last and self.is_decoder:
            self.ffn = KANLinear(self.dim, self.dim, grid_size=5, spline_order=3, scale_spline=0.1)
        else:
...
# Updated make_masks in TransformerModel
    def make_masks(self, src, trg=None):
        src_len = src.size(1)
        src_mask = (src != self.pad_idx).unsqueeze(1).unsqueeze(2)  # [bs, 1, src_len, 1]
        if trg is not None:
            trg_len = trg.size(1)
            trg_pad_mask = (trg != self.pad_idx).unsqueeze(1).unsqueeze(3)  # [bs, 1, trg_len, 1]
            trg_sub_mask = torch.tril(torch.ones(trg_len, trg_len, device=trg.device)).bool()
            trg_mask = trg_pad_mask & trg_sub_mask  # [bs, 1, trg_len, trg_len]
            # Ensure trg_mask has the correct shape
            if trg_mask.size(2) != trg_len or trg_mask.size(3) != trg_len:
                raise ValueError(f"trg_mask shape {trg_mask.shape} does not match expected {trg_len}x{trg_len}")
            return src_mask, trg_mask
        return src_mask, None

    # Updated TransformerModel.forward
    def forward(self, src, trg):
        src_mask, _ = self.make_masks(src)  # Encoder mask based on src only
        _, trg_mask = self.make_masks(src, trg[:, :-1])  # Decoder mask based on trg[:, :-1]
        enc_out = self.embedding(src)
        for i, layer in enumerate(self.layers):
            if i < self.params.n_enc_layers:
                enc_out = layer(enc_out, src_mask, None, None)
            else:
                dec_in = self.embedding(trg) if i == self.params.n_enc_layers else enc_out
                dec_out = layer(dec_in, trg_mask, src_mask, enc_out)
                enc_out = dec_out
        logits = self.proj(dec_out).clamp(-1e6, 1e6)
        return logits

    # def forward(self, src, trg):
    #     src_mask, trg_mask = self.make_masks(src, trg[:, :-1])  # Use trg[:, :-1] for mask generation
    #     enc_out = self.embedding(src)
    #     for i, layer in enumerate(self.layers):
    #         if i < self.params.n_enc_layers:
    #             enc_out = layer(enc_out, src_mask, None, None)
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} (Train)"):
            ...
            if torch.isnan(loss).any():
                ...
                continue
            # Add KAN regularization loss
            kan_reg_loss = 0
            for layer in model.layers:
                if hasattr(layer.ffn, 'regularization_loss'):
                    kan_reg_loss += layer.ffn.regularization_loss(regularize_activation=0.01, regularize_entropy=0.01)
            total_loss = loss + kan_reg_loss
            total_loss.backward()
