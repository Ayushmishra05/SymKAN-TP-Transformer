...
# S-KANformer Components (unchanged)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, d_model: int, d_ff: int, dropout: float, w0: float = 1.0) -> None:
        super().__init__()
        self.w0 = w0
    ...
    def init_weights(self):
        with torch.no_grad():
            # SIREN-style initialization for linear_1
            self.linear_1.weight.uniform_(
                -math.sqrt(6 / self.linear_1.in_features) / self.w0,
                 math.sqrt(6 / self.linear_1.in_features) / self.w0
            )

    def forward(self, x):
        return self.linear_2(self.dropout(torch.sin(self.w0 * self.linear_1(x))))
...
def main():
    ...
    # Replace the training loop section in main()
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} (Train)"):
            ...
            # Generate tgt_mask based on trg[:, :-1] length
