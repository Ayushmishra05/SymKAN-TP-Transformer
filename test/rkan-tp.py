...
# Jacobi Polynomial Implementations
def shifted_jacobi_polynomial(x, n, alpha, beta, zeta, a, b, backend=torch):
    ...
    if n == 0:
        ...
        return (alpha + beta + 2) * x + (alpha - beta) / 2
    else:
        p0 = torch.ones_like(x, device=x.device)
        p1 = (alpha + beta + 2) * x + (alpha - beta) / 2
        for i in range(2, n + 1):
            a_n = 2 * i * (i + alpha + beta) * (2 * i + alpha + beta - 2)
            b_n = (2 * i + alpha + beta - 1) * (alpha + beta + i) * (2 * i + alpha + beta)
            c_n = 2 * (i - 1 + alpha) * (i - 1 + beta) * (2 * i + alpha + beta)
            p2 = ((b_n * x + c_n) * p1 - a_n * p0) / (a_n + b_n + c_n)
            p0, p1 = p1, p2
        return p1 * torch.sqrt((alpha + beta + 1) / 2)

def rational_jacobi_polynomial(x, n, alpha, beta, scale, iota, backend=torch):
    base_poly = shifted_jacobi_polynomial(x, n, alpha, beta, 0, 0, 1, backend)
    return base_poly / (1 + iota * torch.abs(x)) * scale
    ...
    def forward(self, inputs):
        ...
        for deg in range(2, self.degree_p):
            p += self.w_p[deg] * shifted_jacobi_polynomial(
                normalized_inputs, deg, normalized_alpha_p, normalized_beta_p, normalized_zeta_p, 0, 1, backend=torch
            )
        q = self.w_q[0] + self.w_q[1] * normalized_inputs
        for deg in range(2, self.degree_q):
            q += self.w_q[deg] * shifted_jacobi_polynomial(
                normalized_inputs, deg, normalized_alpha_q, normalized_beta_q, normalized_zeta_q, 0, 1, backend=torch
            )
        return p / q
...
class JacobiRKAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return rational_jacobi_polynomial(inputs, self.degree, normalized_alpha, normalized_beta, 1, normalized_iota, backend=torch)

# RKANLayer
class RKANLayer(nn.Module):
    def __init__(self, in_features, out_features, degree_p=3, degree_q=3, use_pade=True, device='cuda'):
        super(RKANLayer, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.use_pade = use_pade
        self.device = device
        self.linear = nn.Linear(in_features, out_features)
        if use_pade:
            self.rkan = PadeRKAN(degree_p, degree_q)
        else:
            self.rkan = JacobiRKAN(degree_p)
...
class KANFeedForwardBlock(nn.Module):
    def __init__(self, in_size: int, ff_dims: List[int], grid_size: int = 8, device: Union[str, int] = 'cuda') -> None:
        ...
        for i, d in enumerate(ff_dims):
            self.ffn.append(RKANLayer(
                in_size, d, degree_p=3, degree_q=3, use_pade=True, device=device))
            in_size = d
...
# Your Tokenization and Dataset (integrated)
