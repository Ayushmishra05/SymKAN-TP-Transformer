...
class KANFeedForwardBlock(nn.Module):
    def __init__(self, in_size: int, ff_dims: List[int], gridsize: int = 4, device: Union[str, int] = 'cuda') -> None:
        super().__init__()
        self.ffn = nn.ModuleList()
        for i, d in enumerate(ff_dims):
            self.ffn.append(NaiveFourierKANLayer(
                inputdim=in_size, outdim=d, gridsize=gridsize, addbias=True, smooth_initialization=False))
            in_size = d
...
def build_kanformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int=256, 
                    N: int=3, h: int=8, dropout: float=0.1, d_ff: int=4096, ff_dims: List[int]=[2048], device: Union[str, int] = 'cuda') -> Transformer:
...
def main():
    ...
    dropout = 0.1
    d_ff = 4096
    ff_dims = [2048]  # Reduced from [8192]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    max_length = 100  # Reduced from 300
    epochs = 1
    lr = 1e-4
    batch_size = 4
...
# Inference
    ...
    src_tokens = tokenizer.src_tokenize(test_expr)[:max_length]  # Truncate to max_length
    ...
    print(f"Input: {test_expr[:100]}... (truncated to 100 tokens)")
