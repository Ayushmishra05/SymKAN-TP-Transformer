...
tokenizer = Tokenizer(df, index_token_pool_size, special_symbols, UNK_IDX, to_replace)

# Build vocabularies
src_vocab_set = tokenizer.build_src_vocab()
tgt_vocab_set = tokenizer.build_tgt_vocab()

print("==" * 30, src_vocab_set , len(src_vocab_set))

# Initialize vocab objects
src_vocab = Vocab(src_vocab_set, special_symbols, BOS_IDX, PAD_IDX, EOS_IDX, UNK_IDX, SEP_IDX)
tgt_vocab = Vocab(tgt_vocab_set, special_symbols, BOS_IDX, PAD_IDX, EOS_IDX, UNK_IDX, SEP_IDX)

# Tokenize and encode
src_tokenized = [tokenizer.src_tokenize(amp) for amp in df['amp']]
tgt_tokenized = [tokenizer.tgt_tokenize(sqamp) for sqamp in df['sqamp']]
src_encoded = [[src_vocab.bos_idx] + src_vocab.encode(tokens) + [src_vocab.eos_idx] for tokens in src_tokenized]
tgt_encoded = [[tgt_vocab.bos_idx] + tgt_vocab.encode(tokens) + [tgt_vocab.eos_idx] for tokens in tgt_tokenized]

# Print an example
print(f"Example source tokens: {src_tokenized[0]}")
print(f"Example target tokens: {tgt_tokenized[0]}")
print(f"Example source encoded: {src_encoded[0]}")
print(f"Example target encoded: {tgt_encoded[0]}")